...
1. Enter `n` (New remote) and name it `my_box`.
2. For storage, select `box`.
3. Press Enter to leave the next few options blank, and for `box_sub_type`, enter `user`.
4. For **Use auto config?** select `y`. A browser window will pop up.
5. Log in to your Purdue Box account and grant access. Return to the terminal and enter `q` to quit.
